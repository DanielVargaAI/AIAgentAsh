...
class PokerogueExtractor:
    """
    V2.7 Extractor for PokeRogue.
    - NEW: Saves Debug Screenshots & JSON Data to a folder.
    - ROI: Center 60% Crop for Types.
    - COLOR: Calibrated V2.5 values.
    """
    ...
    def _identify_types(self, roi, debug_mode=False):
        """V2.6: Center 60% Crop + Median Color Logic."""
        if roi.size == 0: return "Unknown"
        h, w = roi.shape[:2]

        # 1. Center Crop (Remove outer 20%)
        margin_y = int(h * 0.20) 
        margin_x = int(w * 0.20)
        ...
        # 2. Analyze Top/Bottom of cropped area
    ...
    def save_snapshot(self, state, debug_image, folder="pokerogue_data"):
        """
        Saves the current game state (JSON) and Debug Image to a folder.
        """
        ...
        # 1. Save Image
        img_filename = f"{folder}/state_{timestamp}.png"
        if debug_image is not None:
            cv2.imwrite(img_filename, debug_image)
            print(f"Saved Image: {img_filename}")

        # 2. Save JSON Data (Vector Source)
        json_filename = f"{folder}/state_{timestamp}.json"
...
if __name__ == "__main__":
    ...
    if img is not None:
        ...
        # --- NEW: SAVE THE DATA ---
        extractor.save_snapshot(state, dbg)

        cv2.imshow("V2.7 Final", dbg)
        cv2.waitKey(0)
