...
try:
    ...
    from . import phase_handler_0912_learningmove as phase_handler
...
class PokeRogueEnv(gym.Env):
    def __init__(self):
        ...
        _, reward_meta, reward_obs = phase_handler.phase_handler(self.new_meta_data, self.new_obs, self.driver, self.pokemon_embeddings_data,
                                                                 self.move_embeddings_data, phase_counter=0, terminated=False, ongoing_save=False)
    ...
    def step(self, action):
        ...
        time.sleep(0.5)  # Wait a bit for the game to process the action
        self._get_obs()
        time.sleep(0.5)  # Wait a bit for the new observation to be ready
        ...
        time.sleep(0.5)  # Wait a bit before returning
    ...
    def _get_obs(self):
        # Default meta_data with all required keys
        default_meta_data = {
            "phaseName": "UNKNOWN",
            "stage": 0,
            "hp_values": {"enemies": {}, "players": {}},
            "is_double_fight": False,
            "shop_items": []
        }
        ...
        try:
            ...
            if raw_data is None or type(raw_data) is not dict or 'enemy' not in raw_data:
                self.new_meta_data = default_meta_data
